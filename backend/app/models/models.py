...
class TrackedPrompt(Base):
    ...
    intent: Mapped[str | None] = mapped_column(String(30), nullable=True)  # informational, commercial, comparison, conversational, navigational
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
...
class Action(Base):
    """Persisted AEO optimization action generated by the Action Center."""
    ...
    category: Mapped[str] = mapped_column(String(50), nullable=False)  # website_audit, crawler_analysis, content_gap, etc.
    ...
    # Optional metadata (varies by category)
    ...
    current_mention_rate: Mapped[float | None] = mapped_column(Float)
    current_rate: Mapped[float | None] = mapped_column(Float)
    suggested_content: Mapped[str | None] = mapped_column(Text)
    suggested_schema: Mapped[str | None] = mapped_column(Text)

    # Verification
    verification_type: Mapped[str | None] = mapped_column(String(30))
    baseline_value: Mapped[float | None] = mapped_column(Float)
    verified_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    verified_value: Mapped[float | None] = mapped_column(Float)
    verification_status: Mapped[str | None] = mapped_column(String(20))

    # Crawler / audit metadata
    crawler_type: Mapped[str | None] = mapped_column(String(50))
    audit_category: Mapped[str | None] = mapped_column(String(50))
    audit_severity: Mapped[str | None] = mapped_column(String(20))

    # Engine breakdown (for content gaps)
    engines_missing: Mapped[dict | None] = mapped_column(JSONB)  # list of engine names
    engines_citing: Mapped[dict | None] = mapped_column(JSONB)   # list of engine names

    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
    updated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))

    __table_args__ = (
        Index("ix_actions_brand_status", "brand_id", "status"),
    )
...
class AuditResult(Base):
    """Persisted AEO audit result for historical tracking."""
    ...
    schema_suggestions: Mapped[dict | None] = mapped_column(JSONB)
    llms_txt_content: Mapped[str | None] = mapped_column(Text)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)

    __table_args__ = (
        Index("ix_audit_results_url_ts", "url", "created_at"),
        Index("ix_audit_results_brand", "brand_id", "created_at"),
    )
